...
/**
 * Subscribe to or unsubscribe from a sforum discussion.
 *
 * @package    mod_sforum
...
$sforumid = required_param('sforumid', PARAM_INT); // The sforum to subscribe or unsubscribe
...
$course         = $DB->get_record('course',
        array('id' => $sforum->course), '*', MUST_EXIST);
if (!$discussion = $DB->get_record('sforum_discussions',
        array('id' => $discussionid, 'sforum' => $sforumid))) {
...
require_capability('mod/sforum:viewdiscussion', $context);
...
if (is_guest($context, $USER)) {
    ...
    throw new moodle_exception('noguestsubscribe', 'mod_sforum');
}
...
if (\mod_sforum\subscriptions::is_subscribed($USER->id,
    ...
    \mod_sforum\subscriptions::unsubscribe_user_from_discussion($USER->id,
                $discussion, $context);
} else {
...
$return->icon = sforum_get_discussion_subscription_icon($sforum,
