...
class scripting_forum_portfolio_caller extends portfolio_module_caller_base {
    ...
    function __construct($callbackargs) {
        ...
        if (!$this->postid && !$this->discussionid) {
            throw new portfolio_caller_exception('mustprovidediscussionorpost', 'scripting_forum');
        }
    }
    /**
     * @global object
    ...
    public function load_data() {
        ...
        if ($this->postid) {
            if (!$this->post = $DB->get_record('scripting_forum_posts', array('id' => $this->postid))) {
                throw new portfolio_caller_exception('invalidpostid', 'scripting_forum');
            }
        }
        ...
        if (!$this->discussion = $DB->get_record('scripting_forum_discussions', $dbparams)) {
        ...
        if (!$this->cm = get_coursemodule_from_instance('scripting_forum', $this->scripting_forum->id)) {
    ...
    function get_return_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/scripting_forum/discuss.php?d=' . $this->discussion->id;
    }
    /**
     * @global object
     * @return array
    ...
    function get_navigation() {
        ...
        $navlinks[] = array(
            ...
            'link' => $CFG->wwwroot . '/mod/scripting_forum/discuss.php?d=' . $this->discussion->id,
            'type' => 'title'
        );
    ...
    function prepare_package() {
        ...
        if ($this->attachment) { // simplest case first - single file attachment
            ...
            if ($writingleap) {
                ...
                $selection = new portfolio_format_leap2a_entry('scripting_forumdiscussion' . $this->discussionid,
                    get_string('discussion', 'scripting_forum') . ': ' . $this->discussion->name, 'selection');
     ...
     * that corresponds to a single scripting_forum post,
    ...
    /**
     * this is a very cut down version of what is in scripting_forum_make_mail_post
     *
     * @global object
     * @param int $post
     * @return string
    ...
    private function prepare_post($post, $fileoutputextras=null) {
        ...
        $output .= '<div class="author">'.get_string('bynameondate', 'scripting_forum', $by).'</div>';
    ...
    function check_permissions() {
        ...
        if ($this->post) {
            return (has_capability('mod/scripting_forum:exportpost', $context)
                || ($this->post->userid == $this->user->id
                    && has_capability('mod/scripting_forum:exportownpost', $context)));
        }
        return has_capability('mod/scripting_forum:exportdiscussion', $context);
    }
    /**
     * @return string
     */
    public static function display_name() {
        return get_string('modulename', 'scripting_forum');
    }
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class scripting_forum_file_info_container extends file_info {
    /** @var file_browser */
    protected $browser;
    /** @var stdClass */
    protected $course;
    ...
    public function __construct($browser, $course, $cm, $context, $areas, $filearea) {
        ...
        $this->course = $course;
        $this->cm = $cm;
        $this->component = 'mod_scripting_forum';
